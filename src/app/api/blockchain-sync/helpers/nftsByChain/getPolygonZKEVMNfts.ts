...
export async function getPolygonZKEVMNfts(walletAddress: string, contracts: string[]) {
    const data = await axios<TransactionsResponse>('https://api-zkevm.polygonscan.com/api', {
      params: {
        module: 'account',
        action: 'txlist',
        address: walletAddress,
        sort: 'desc',
        apikey: 'WBQGM99IEYX63E52Y92ST17TTZVXV9DEMG',
      },
    });
    ...
    for (const nft of data.data.result) {
        ...
        const txData = await axios<TokensResponse>('https://api-zkevm.polygonscan.com/api', {
          params: {
            module: 'account',
            action: 'tokennfttx',
