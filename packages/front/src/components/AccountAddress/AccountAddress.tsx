...
export default function AccountAddress({ address, withCopy, className }) {
    ...
    const handleCopy = () => {
        ...
        messageApi.info('Wallet address has copied!');
    };
    ...
    return (
        <Flex gap={4} className={clsx(styles.accountAddress, className)}>
            {contextHolder}
            {value}
            {withCopy &&  <IconBtn tooltip="Copy" onClick={handleCopy}>
